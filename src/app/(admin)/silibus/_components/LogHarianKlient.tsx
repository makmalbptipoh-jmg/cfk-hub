...
export function LogHarianKlient({ cawanganAwal }: { cawanganAwal: Cawangan[] }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', maxWidth: '620px' }}>
          Log harian: satu baris = satu tajuk yang diajar pada satu tarikh untuk satu kelas. Untuk kurikulum berstruktur &amp; progress, guna tab <strong>Silibus Induk</strong>.
        </p>
        <button
            ...
            <tbody>
              {senarai.map((r, i) => (
                <tr key={r.id} style={{ borderTop: i > 0 ? '1px solid var(--border)' : 'none' }}>
                  <td style={{ padding: '15px 16px', fontSize: '13px', fontWeight: 600, color: 'var(--text)', whiteSpace: 'nowrap' }}>{formatTarikh(r.tarikh)}</td>
                  <td style={{ padding: '15px 16px', fontSize: '12.5px', color: 'var(--text-muted)' }}>{HARI[hariMinggu(r.tarikh)]}</td>
                  <td style={{ padding: '15px 16px', fontSize: '13px', color: 'var(--text)' }}>
                  ...
                  <td style={{ padding: '15px 16px', fontSize: '13px', fontWeight: 600, color: 'var(--text)', minWidth: '260px', lineHeight: 1.5 }}>
                    {r.tajuk}
                    {r.nota && <div style={{ fontSize: '11.5px', fontWeight: 400, color: 'var(--text-muted)', marginTop: '3px', lineHeight: 1.5 }}>{r.nota}</div>}
                  </td>
                  <td style={{ padding: '15px 16px', fontSize: '13px', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{r.muka_surat ?? '—'}</td>
