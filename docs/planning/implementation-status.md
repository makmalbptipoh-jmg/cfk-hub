...
### Modul Penggredan (Grading) — LANGKAH 1 asas SIAP (typecheck LULUS; 36 ujian unit lulus; BELUM run SQL, BELUM diuji browser)
Keperluan user: semak `Downloads/GRADING_SPEC.md` + buat pelan + cadang penambahbaikan. Spec ditulis untuk seni bina **Google Sheets + HTML tunggal + Apps Script + CDN (jsPDF/html2canvas/SheetJS)** — TAPI CFK HUB ialah Next.js/Supabase. Keputusan user (via soalan): (1) **integrasi ke app sedia ada** (bukan standalone); (2) hasilkan **pelan pembinaan penuh**; (3) skor Improvement **guna semula rating modul Pertandingan** (`pertandingan.ts`, Elo asas 1000). Pelan penuh: `~/.claude/plans/c-users-user-downloads-grading-spec-md-a-snoopy-diffie.md`.
- **Penambahbaikan vs spec:** terjemah Sheets→Postgres+RLS, Apps Script→Supabase; buang jsPDF/html2canvas/SheetJS → guna `@react-pdf/renderer`+`exceljs` sedia ada; jangan duplikasi `cawangan`/`pelajar`/`kehadiran`/`pertandingan` (FK + auto-derive); reconcile TIGA sistem "Pawn→King" (rating.ts bintang / pertandingan.ts rating / grading tahap silibus) via awalan label; petakan palet spec `#1E63D5` → var tema `--primary #1E293B`/`--accent #84CC16`.
- ⚠️ **WAJIB run `scripts/sql/penggredan.sql` SEBELUM guna UI** (3 jadual: `gred_kitaran`, `gred_penilaian`, `gred_little_pawn`; idempotent + ROLLBACK; RLS baca terbuka, tulis kitaran=admin, tulis penilaian/little_pawn=admin ATAU jurulatih). Selepas run: sahkan `pg_policies` (6 baris) + run `_semakan-policy-audit.sql` → 0 baris (elak landmine 42501 Sesi 20). Kemudian seed 1 baris `gred_kitaran` untuk suku semasa.
- **Fail baharu:** `scripts/sql/penggredan.sql`; `src/lib/grading.ts` (Level 1-6: skor berpemberat 25/20/25/10/10/10+5, gred A-E, gate promosi dgn sebab, `TARAF_GRED`, band umur, guna semula `kiraRingkasanPertandingan`, `namaFail`); `src/lib/gradingLittlePawn.ts` (Level 0: checklist 12-item, peringkat/graduasi, + seed const `AKTIVITI_LITTLE_PAWN` 8 aktiviti/`SESI_LITTLE_PAWN` 7 slot/`JADUAL_LITTLE_PAWN` 12 minggu, `mingguSemasa`); + `grading.test.ts` & `gradingLittlePawn.test.ts` (36 ujian lulus).
- **Diubah:** `src/types/database.ts` (+3 blok jenis), `scripts/sql/_semakan-policy-audit.sql` (+6 baris polisi grading).

### Modul Penggredan — LANGKAH 2-4 SIAP (typecheck+lint+build LULUS; 41 ujian unit; BELUM diuji browser)
Keputusan tambahan user: `club_points` **manual** (tiada jadual club-ladder); nav jurulatih guna **menu overflow "…"** (bila S9/S10 dibina, L7). Corak seni bina: modul letak dalam route group **`(jurulatih)`** sahaja (macam `/pertandingan`) — layout `(jurulatih)` beri chrome Sidebar utk admin & BottomTabBar utk jurulatih, jadi satu route `/penggredan` layan kedua-dua peranan.
- **L2 — S1 Dashboard** (`(jurulatih)/penggredan/page.tsx` + `_components/PenggredanDashboardKlient.tsx`): dropdown kitaran+cawangan (admin=Semua, jurulatih=cawangan sendiri via `jurulatih.cawangan_ids`), 4 kad stat (jumlah/dinilai/belum/naik level), carta bar taburan gred A-E (div SVG-style), jadual pelajar (Tahap Silibus ikon+nama, status, skor, gred, butang Nilai→route S2/S6 ikut level). Level semasa derive guna `levelSemasaPelajar` (rekod penilaian terbaru / Little Pawn graduated / umur). Nav: `Sidebar.tsx` +item "/penggredan" (ikon `Award`) selepas Pertandingan.
- **L3 — S2 Borang Penilaian** (`nilai/[pelajarId]/page.tsx` + `_components/BorangPenilaianKlient.tsx` + action `src/app/actions/penggredan.ts::simpanPenilaian`): auto-derive kehadiran dari `kehadiran` dalam julat tarikh kitaran (kecuali Cuti); rating_tamat auto dari `pertandingan_keputusan` (`ratingSemasaDariKeputusan`), rating_mula dari `rating_tamat` kitaran terdahulu (atau 1000); live preview skor+gred+bar komponen; amaran merah promosi (`sebabTakNaik`); auto-save draf (debounce 900ms) + butang Tandakan Selesai. **Skor/gred/naik_level dikira di SERVER** (action guna `kiraPenilaian`) — sumber kebenaran tunggal. Action kunci edit bila kitaran Ditutup.
- **L4 — S6 Checklist Little Pawn** (`little-pawn/[pelajarId]/page.tsx` + `_components/ChecklistLittlePawnKlient.tsx` + action `simpanLittlePawn`): 12 item dlm 3 kumpulan Kenal/Gerak/Main, butang 3-status (Belum/Sedang/Dah Boleh), progress bar "N/12" + bintang peringkat, popup "?" aktiviti latihan per item (`aktivitiUntukItem`), kehadiran/sikap/minigame, auto-save + Selesai. Server kira peringkat/graduasi.
- **Fail baharu L2-4:** `page.tsx`×3, `PenggredanDashboardKlient.tsx`, `BorangPenilaianKlient.tsx`, `ChecklistLittlePawnKlient.tsx`, `src/app/actions/penggredan.ts`; **+helper `levelSemasaPelajar`** dalam `grading.ts` (+5 ujian). **Diubah:** `Sidebar.tsx`.

### Modul Penggredan — L5 + E1 Kad Laporan L1-6 SIAP (typecheck+lint+build LULUS; BELUM diuji browser)
SQL `penggredan.sql` **sudah di-run user di Supabase** — audit `_semakan-policy-audit.sql` pulang 0 baris (semua 6 polisi grading wujud, tiada landmine 42501). ⚠️ user masih perlu **seed 1 `gred_kitaran`** untuk uji UI.
- **L5 S3 Kad Laporan (Level 1-6)** — route baharu `(jurulatih)/penggredan/kad/[pelajarId]/page.tsx` (server: fetch pelajar+cawangan+kitaran+rekod, kira semula 6 komponen guna `kiraPenilaian`, fokus = komponen ratio terendah) + `_components/KadPenilaianPreview.tsx` (kad skrin: header CFK, info, bar pecahan markah, bulatan gred berwarna, status NAIK/KEKAL + delta rating, komen coach, fokus; butang Edit + Muat Turun PDF; amaran bila Draf).
- **E1 PDF** — `src/components/pdf/KadGredPDF.tsx` (@react-pdf: header/logo, info, pecahan bar, **radar chart 6-komponen guna `Svg/Polygon/Line/Circle`**, bulatan gred, status, komen, fokus, footer tandatangan; `WARNA_GRED_PDF`) + `src/components/pdf/BtnKadGredPDF.tsx` (dynamic import `pdf().toBlob()`, nama fail `CFK_Report_[Nama]_[Kitaran].pdf` via `namaFail`, guna `toast`). Line chart rating 4-kitaran **ditunda** (perlu fetch sejarah — buat di L8/S5).
- **Diubah:** `PenggredanDashboardKlient.tsx` (+pautan "Kad" pada baris L1-6 status≠Belum).
- **Tertunggak (ikut pelan):** L6 baki — **S7+E2** Kad Little Pawn + Sijil Pawn (A4 landscape), **E4** Excel 5-sheet → L7 S8-S11 sesi/aktiviti + nav overflow jurulatih → L8 S4 Batch/S5 Sejarah (+line chart rating)/E3 Laporan Kelas/E5 (JSZip dep). Soalan terbuka: SMK STAR cawangan ke-5, JSZip dep (E5).
- **Ujian tambahan (preview, selepas seed kitaran + nilai 1 pelajar Selesai):** /penggredan → baris L1-6 Selesai papar butang "Kad" → buka kad skrin (radar/gred/status betul) → Muat Turun PDF buka fail; Edit kembali ke borang.

### Modul Penggredan — L6 LENGKAP: S7+E2 Kad Little Pawn + Sijil + E4 Excel (typecheck+lint+build LULUS; BELUM diuji browser)
Semua kad laporan + eksport data untuk KEDUA-DUA sistem kini siap. 5 route penggredan: page, nilai/, little-pawn/, kad/, kad-lp/.
- **S7 Kad Little Pawn** — route `kad-lp/[pelajarId]/page.tsx` (fetch rekod, bina 12 item + bintang peringkat + fokus 2 item pertama <2 + nama aktiviti sepadan) + `_components/KadLittlePawnPreview.tsx` (visual: bintang, grid 12-item warna status, kata coach, fokus; **TIADA gred/%**). Dashboard: butang "Kad" kini route ke `kad-lp` utk Level 0, `kad` utk L1-6.
- **E2 PDF** — `KadLittlePawnPDF.tsx` (A4 portrait visual, bintang ★, grid item warna Belum/Sedang/Dah Boleh, legenda) + `SijilPawnPDF.tsx` (**A4 landscape** sijil Pawn Promotion, bingkai dwi-warna, hanya bila graduasi) + `BtnKadLittlePawn.tsx` (butang "Muat Turun Report" + "Muat Turun Sijil" bila graduasi).
- **E4 Excel** — `src/components/excel/BtnDataGredExcel.tsx` (fetch sendiri via supabase client, corak `BtnLaporanAset`): **5 sheet** — Ringkasan (kira komponen guna `kiraPenilaian`, conditional fill gred), Data Mentah (semua field + autofilter, audit), Little Pawn (i01-i12 sebagai teks Belum/Sedang/Dah Boleh), Statistik Cawangan (per cawangan + baris SEMUA CAWANGAN), Sejarah Progress (semua kitaran, susun nama→cycle). Header `#1E293B` putih, freeze row 1. Butang dalam header dashboard (tapis ikut kitaran+cawangan terpilih).
- **Fail baharu:** `KadLittlePawnPDF.tsx`, `SijilPawnPDF.tsx`, `BtnKadLittlePawn.tsx`, `BtnDataGredExcel.tsx`, route `kad-lp/[pelajarId]`, `KadLittlePawnPreview.tsx`. **Diubah:** `PenggredanDashboardKlient.tsx` (+butang Excel, +route kad-lp/kad ikut level).
- **Tertunggak:** L7 S8-S11 modul aktiviti/sesi coach (Panduan Aktiviti, Pelan Sesi timer 90-min, Ringkasan+WhatsApp, Jadual 12-minggu) + nav overflow "…" jurulatih → L8 S4 Batch Entry, S5 Sejarah (+line chart rating), E3 Laporan Kelas PDF, E5 (JSZip dep). Soalan terbuka: SMK STAR cawangan ke-5, JSZip dep (E5).
- **Ujian (preview, selepas nilai pelajar):** L1-6 → Kad → radar+PDF; Level 0 → Kad → bintang+grid+Report PDF, bila 12/12 → butang Sijil (landscape); dashboard → butang Excel → 5 sheet betul (Ringkasan conditional gred, Little Pawn teks, Statistik SEMUA CAWANGAN).

### Modul Penggredan — L7 LENGKAP: modul aktiviti/sesi coach S8-S11 + nav overflow (typecheck+lint+build LULUS; BELUM diuji browser)
Semua const-driven (`AKTIVITI_LITTLE_PAWN`/`SESI_LITTLE_PAWN`/`JADUAL_LITTLE_PAWN`/`mingguSemasa`) — TIADA SQL baharu. 8 route penggredan.
- **S8 Panduan Aktiviti** (`aktiviti/page.tsx` + `PanduanAktivitiKlient.tsx`): 8 kad aktiviti boleh kembang (bahan/langkah bernombor/coach tip kotak kuning), butang "Mula Aktiviti" → **timer countdown** `durasiMin` + mini-checklist item di-cover.
- **S9 Pelan Sesi Hari Ini + S10 Ringkasan** (`sesi/page.tsx` server tarik kitaran Dibuka+cawangan jurulatih → `PelanSesiKlient.tsx`): header minggu semasa + Aktiviti A/B auto dari jadual; 7 slot kad warna ikut jenis (ritual/activity/break/assess); **butang "Mula Sesi" → timer 90-min**, auto-highlight slot semasa (ikut menit elapsed), **chime Web Audio** bila tukar slot, auto-scroll ke slot tengah, countdown baki slot, tap slot lompat manual; "Tamat Sesi" → **S10 Ringkasan**: bilangan hadir + nota parent (maks 140) + **share WhatsApp** (`wa.me/?text=`).
- **S11 Jadual Kelas** (`jadual-little-pawn/page.tsx` server tarik kitaran Dibuka → `JadualLittlePawnKlient.tsx`): timeline 12 minggu, highlight minggu semasa (`#1E63D5`), tap minggu → papar Aktiviti A/B + pautan buka.
- **Nav jurulatih (overflow):** `BottomTabBar.tsx` — Makluman dipindah ke sheet "Lagi" (kekal 5 tab + Lagi + Keluar); butang **"Lagi" (⋯)** buka sheet bawah: Penggredan, Pelan Sesi Hari Ini, Panduan Aktiviti, Jadual Kelas, Makluman.
- **Tertunggak:** L8 — S4 Batch Entry, S5 Sejarah (+line chart rating guna `GrafProgres`), E3 Laporan Kelas PDF (multi-page), E5 PDF Gabung (+zip perlu keputusan JSZip). Soalan terbuka: SMK STAR cawangan ke-5, JSZip dep (E5).
- **Ujian (preview):** jurulatih → tab Lagi → Pelan Sesi → Mula Sesi → slot auto-highlight+chime+countdown, tap lompat, Tamat → ringkasan+WhatsApp; Panduan Aktiviti → timer; Jadual → minggu semasa highlight.

### Modul Penggredan — L8 LENGKAP: S4 Batch, S5 Sejarah, E3 Laporan Kelas (MODUL SELESAI; typecheck+lint+build LULUS; 41 ujian; BELUM diuji browser)
**Semua langkah pelan (L1-L8) kini siap.** 10 route penggredan. Hanya E5 zip-individu tertunggak (perlu keputusan `npm i jszip`) — E3 sudah beri satu PDF gabungan semua kad (memenuhi keperluan "PDF Gabung").
- **S4 Batch Entry** (`batch/page.tsx` server: pelajar+kitaran+cawangan+rekod+rating pertandingan semua → `_components/BatchEntryKlient.tsx`): grid boleh-edit (Lvl/Theory/Puzzle/Club/Tourn/Hadir/Sikap×4/Bonus), Tab antar sel, gred langsung per baris, tapis kitaran+cawangan, prapopulasi rekod sedia ada, bar melayang "Simpan Draf"/"Simpan Semua (Selesai)". Action baharu `simpanBatch` (upsert pukal, kira server). Improvement guna rating pertandingan semasa (delta 0 dlm batch — perhalusi di S2).
- **S5 Sejarah Progress** (`sejarah/[pelajarId]/page.tsx` + `SejarahKlient.tsx`): guna semula `GrafProgres` — line chart Final Score + Rating Pertandingan merentas kitaran + timeline naik level. Pautan "Sejarah" ditambah di kad laporan L1-6.
- **E3 Laporan Kelas PDF** (`LaporanKelasGredPDF.tsx` + `BtnLaporanKelasPDF.tsx`): multi-page — muka depan (logo, jumlah, taburan gred, naik level), jadual ringkasan (baris berwarna ikut gred), satu kad penuh per pelajar (pecahan bar+bulatan gred+status+komen+fokus). Butang dalam dashboard (tapis kitaran+cawangan, hanya status Selesai).
...
### Jurulatih update Silibus Pelajar dari telefon (LIVE production; SQL `silibus-jurulatih.sql` di-run pada 2026-08-21)
...
- ⚠️ **WAJIB run `scripts/sql/silibus-jurulatih.sql` SEBELUM deploy** — 2 polisi RLS baharu pada `silibus_progress_pelajar`: `jurulatih_insert_*` + `jurulatih_update_*` dengan `jurulatih_id_semasa() IS NOT NULL` (INSERT+UPDATE sahaja; polisi admin FOR ALL dikekalkan; jurulatih tak boleh DELETE). Guna semula `jurulatih_id_semasa()`. Idempotent + ROLLBACK. **Tiada jadual baharu** — jadual `silibus_progress_pelajar` dikongsi admin & jurulatih (satu sumber kebenaran).
  - 🐞 **Insiden 2026-08-21:** kod deploy tetapi SQL ini **tak pernah di-run di production** — jurulatih dapat `42501 new row violates row-level security policy` bila simpan dari telefon (admin di laptop OK sebab polisi `tulis_admin` FOR ALL). Disahkan via `pg_policies`: hanya `baca_*` + `tulis_admin_*` wujud, dua polisi `jurulatih_*` tiada. Diselesaikan dengan run polisi INSERT + UPDATE terus di Supabase SQL Editor (guna nama policy tanpa petik untuk elak isu smart-quote). **Pengajaran:** sahkan `pg_policies` selepas deploy setiap ciri yang bergantung pada migrasi SQL manual.
